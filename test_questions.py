...
def test_ask_user_bool_custom(monkeypatch):
    ...
    assert question.ask_user_bool_custom('K', 'E', False) == (True, 'OK', 0, "Conversion successful")

def test_ask_user_bool_custom_2(monkeypatch):
    user_input = 'E'
    monkeypatch.setattr('builtins.input', lambda _: user_input)
    question = questions.Question('Anna kyllä/ei (K/E): ')
    assert question.ask_user_bool_custom('K', 'E', False) == (False, 'OK', 0, "Conversion successful")

def test_ask_user_bool_custom_3(monkeypatch):
    user_input = 'y'
    monkeypatch.setattr('builtins.input', lambda _: user_input)
    question = questions.Question('Anna kyllä/ei (K/E): ')
    assert question.ask_user_bool_custom('K', 'E', False) == ('N/A', 'Error', 1, 'Unable to convert input to bool')
